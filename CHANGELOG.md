...
- Added an xAI adapter (`Raif::Llms::XAi`) with support for streaming, developer-managed tools, and JSON response formats. Registered Grok models: `x_ai_grok_4_3`, `x_ai_grok_4_20_reasoning`, and `x_ai_grok_4_20_non_reasoning`. Configure via `Raif.config.x_ai_api_key` / `Raif.config.x_ai_models_enabled` (or `ENV["XAI_API_KEY"]`).
- Added xAI [Batch API](https://docs.x.ai/docs/guides/batch) support via `Raif::ModelCompletionBatches::XAi`, implementing the `SupportsBatchInference` contract over xAI's two-call submission flow (`POST /v1/batches` followed by chunked `POST /v1/batches/{id}/requests`). Batch status is derived locally from per-entry counts since xAI has no batch-level state enum.
