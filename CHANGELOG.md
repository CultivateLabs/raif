...
- Added `Raif.config.agent_max_completion_tokens` (default `nil`) to cap output tokens per agent iteration, bounding the cost/latency of runaway generations. When `nil`, each LLM's `default_max_completion_tokens` applies. A response that hits the cap is treated as truncated, discarded, and retried with corrective feedback.
