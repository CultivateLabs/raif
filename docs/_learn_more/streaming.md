...
`bin/smoke` is shipped with Raif for investigating suspected streaming problems with a given model, checking it against the streaming capabilities claimed for it in `lib/raif/model_manifest/definitions/*.rb`:
